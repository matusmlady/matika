...
function scitovanie(){
  const prveCislo = Math.floor(Math.random()*19+1)//20+1
...
function chvostZaciatok(){
  znenie += " = <form action='javascript:check()'><input id='inputVysledok' autocomplete='off'></form>"//size=5 vs css width 100px napriklad
}
...
/////////////////vystup ze sa nieco deje ak napr rovnaky vystup furt dava zlu odpoved tak stale ostane rovnaka hlaska a nevie ci sa stranka zasekla
function check(){///////////////////////tretia tretina kodu de facto duplikat, mozno by sla dat prec s gEBId.includes "dropdown", edit slo by dat prec pocty vstupov premenna celkovo
  let vstupVysledok = document.getElementById("inputVysledok")
  let vystupHlavny = document.getElementById('outputMain')
  function checkInputField(){
    if (vstupVysledok.value != '') {
      if (isNaN(vstupVysledok.value.replace(",",".") )){////////////////////.replace
        vystupHlavny.innerHTML += '<div class="output"><p>Asi máte <em>preklep</em>.</p></div>'
        vstupVysledok.focus()
      } else if (vstupVysledok.value.replace(",",".") == vysledok){
        vystupHlavny.innerHTML += '<div class="output"><p>Príklad bol vypočítaný <em>správne</em>. Super!</p></div>'
        clearTimeout(timer)
        vstupVysledok.blur()
      } else {
        vystupHlavny.innerHTML += '<div class="output"><p>Príklad bol vypočítaný <em>nesprávne</em>. Skúste to ešte raz.</p></div>'
        vstupVysledok.focus()
      }
    } else {
      vstupVysledok.focus()
  ...
  if (document.getElementById('prikladMain').innerHTML.includes('dropdown')){
    if (vstupVysledok.value != '' && document.getElementById('dropdown').value != 1){
      vystupHlavny.innerHTML += '<div class="output"><p>Použite prosím iba <em>jednu</em> formu vstupu.</p></div>'
      } else {
        if (document.getElementById('dropdown').value != 1){
          if (document.getElementById('dropdown').value == 2 && vysledok=='special'){
            ...
            vystupHlavny.innerHTML += '<div class="output"><p>Príklad bol vypočítaný <em>nesprávne</em>. Skúste to ešte raz.</p></div>'
            vstupVysledok.focus()/////////////////////ulozit getelenent by id do premennych
          }
        } else {
          checkInputField()
        }
...
function scitovanieArray(argCislaRaw=cisla){
  ...
  for (let w=0;w<argCisla.length;w++){
    for (let t=argCisla[w].length;t<najdlhsiVstup;t++){argCisla[w].unshift(0)}//doplni za zaciatok nuly aby sa zhodovali dlzky arrayov s cislami
  }
  vysledok=[]
  ...
  for (let z = argCisla[0].length; z>=0; z--){//ratam s tym ze maju vsetky cisla rovnaku dlzku, pre potreby vypoctov dosadim nuly pred cisla
    if (z==0){
      if (zvysok!=0) {vysledok[0]=zvysok;zvysok=0} else {vysledok.shift();break};//zvysok treba vynulovat po zapise
    } else {
      vysledok[z] = zvysok
      for (let x = 0; x<argCisla.length; x++){
        vysledok[z] += argCisla[x][z-1]
      }
      if (vysledok[z]>9){//////////zhoduje s nasobenim nasledujucich sest riadkov
        zvysok = Math.floor(vysledok[z]/10)//zobere aj v pripade ze prenesiem 20 atd nezoberie ako mam 100+ lebo zaokruhli na 10+; co nechcem
...
function nakresliCiaru(xArg,yArg,text='',dlzka=vysledok.length){
  ...
  animacieInstrukcie.push(setTimeout(function(){ctx.fillText(text, xArg-30*vysledok.length, yArg-10)},prestavka+=100))
}
...
function nakresliPoleZozaduRozsirene(argNormal,argZvysok,xArg,yArg){
  ...
  for (let e=argNormal.length-1;e>=0;e--){
    ...
    if (argZvysok[e]!=0 && !(e==0 || (e==1 && argZvysok[1]==argNormal[0]) )){
      animacieInstrukcie.push(setTimeout(function(){ctx.font = "15px Arial"; ctx.fillText(argZvysok[e], xArg+xPosun-10, yArg-15)},prestavka+=prestavkovyCas))
    }
  }
}
...
function postupScitovanie(){
  ...
  prestavka=0
  if (document.getElementById("canvasMain").innerHTML.includes('myCanvas')){/////////////////////////////////////pridat vsade + modre tlacidla zafarbit
    document.getElementById("canvasMain").innerHTML=''
    return
  }
...
function nasobenieViacCiferneho(){////////////////////////////////oddelit tri cifry medzerami kvoli citatelnosti, najma v zneni prikladu
  ...
  for (let z = pocetCifierDruheho-1; z>=0; z--){//let z = 0; z<pocetCifierDruheho; z++
    let zvysok=0
    ...
    if (druhySucinitel[z]!=0){
      if (zvysok!=0) {
        cisla[pocetCifierDruheho-1-z][0]=zvysok;zvysok=0;cislaZvysok[pocetCifierDruheho-1-z].unshift(zvysok)
      } else {
  ...
  neupraveneCisla = cisla.map(daneCislo => (daneCislo.map(danaCifra => danaCifra)))//[...cisla]/////////////
  ...
  for (let w=0;w<cisla.length;w++){//pre kazde cislo=medzivysledok
    for (let g=0;g<w;g++){cisla[w].push(0)}//treba pridat na zaver nuly v druhom riadku jednu atd
  }
...
function postupNasobenie(){
  ...
  document.getElementById("canvasMain").innerHTML='<div class="blok"><canvas  id="myCanvas">Použite prosím novší prehliadač.</canvas></div>'
  c = document.getElementById("myCanvas")
  c.width=vysledok.length*30+40/////////////////////////////dopocitat presnejsie len odhady
  c.height=cisla.length*30+110+60//////////////////////////////////////dopocitat presnejsie len odhady
  ctx = c.getContext("2d")
...
function casovacFunction(){/////////////////tieto hlasky k casovacu su rusive ak maju ostat tak isto aj ine hlasky mozno, dat prec po par sekundach napr po hover
  //zbali menu casovaca
  if (document.getElementById("casovacParagraph").innerHTML.includes('submit')){
    ...
    document.getElementById('outputMain').innerHTML += "<div class='output'><p>Časovač bol vypnutý.</p></div>"
...
function casovacActivate(){//////////////////nove okienko s info o tom ze sa nieco deje ak nie je posledne
  let casovacVstup = document.getElementById('inputCasovac').value.replace(",",".");/////////////////////////////////////
  if (casovacVstup==''){
    document.getElementById("casovacParagraph").innerHTML = "<button id='casovacButton' type='button' title='Nastavte si čas, za ktorý chcete príklad vypočítať.' onclick='casovacFunction()'>Počítanie na čas</button>"
    time=undefined
    clearTimeout(timer)
    document.getElementById('outputMain').innerHTML += "<div class='output'><p>Časovač <em>nebol</em> nastavený.</p>"//casovacVystup.innerHTML=''
    return
  }
  else if (isNaN(casovacVstup)){
    document.getElementById('outputMain').innerHTML += "<div class='output'><p>Asi máte <em>preklep</em>.</p></div>"//casovacVystup.innerHTML='Asi máte <em>preklep</em>.'
    document.getElementById('inputCasovac').focus()
    return
  }
  else if (casovacVstup.includes('.')){
    document.getElementById('outputMain').innerHTML += "<div class='output'><p>Zadajte prosím celé číslo (<em>bez</em> desatinnej čiarky).</p></div>"//casovacVystup.innerHTML='Zadajte prosím celé číslo (<em>bez</em> desatinnej čiarky).'
    document.getElementById('inputCasovac').focus()
    return
  }  
  else if (casovacVstup<=0){
    document.getElementById('outputMain').innerHTML += "<div class='output'><p>V tomto prípade je zbytočné nastavovať časovač.</p></div>"//casovacVystup.innerHTML='V tomto prípade je zbytočné nastavovať časovač.'
    document.getElementById('inputCasovac').focus()
    return
  }  
  else if (casovacVstup<5){
    document.getElementById('outputMain').innerHTML += "<div class='output'><p>Zadajte prosím <em>aspoň</em> 5 sekúnd.</p></div>"//casovacVystup.innerHTML='Zadajte prosím <em>aspoň</em> 5 sekúnd.'
    document.getElementById('inputCasovac').focus()
    return
  }
  
  document.getElementById('outputMain').innerHTML += "<div class='output'><p>Časovač bol úspešne nastavený na "+casovacVstup+"s.</p></div>"//casovacVystup.innerHTML='Časovač bol úspešne nastavený na '+casovacVstup+'s.'
